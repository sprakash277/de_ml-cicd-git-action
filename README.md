...
```
dbx execute --cluster-name=<name of interactive cluster> --job=de_ml-cicd-git-action-sample-integration-test
```

For a test on an automated job cluster, deploy the job files and then launch:
```
dbx deploy --jobs=de_ml-cicd-git-action-sample-integration-test --files-only
...
- To trigger the release pipeline, get the current version from the `de_ml_cicd_git_action/__init__.py` file and tag the current code version:
```
